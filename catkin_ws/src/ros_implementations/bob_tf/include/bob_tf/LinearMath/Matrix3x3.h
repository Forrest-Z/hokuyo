#ifndef	_BOB_TF2_MATRIX3x3_H
#define _BOB_TF2_MATRIX3x3_H
...
namespace bob
{
	...
	class Matrix3x3 {
		...
		void setEulerYPR(tf2Scalar eulerZ, tf2Scalar eulerY,tf2Scalar eulerX)  { 
			tf2Scalar ci ( cos(eulerX)); 
			tf2Scalar cj ( cos(eulerY)); 
			tf2Scalar ch ( cos(eulerZ)); 
			tf2Scalar si ( sin(eulerX)); 
			tf2Scalar sj ( sin(eulerY)); 
			tf2Scalar sh ( sin(eulerZ)); 
			tf2Scalar cc = ci * ch; 
			tf2Scalar cs = ci * sh; 
			tf2Scalar sc = si * ch; 
			tf2Scalar ss = si * sh;
		...
		void getRotation(Quaternion& q) const
		{
			...
			{
				tf2Scalar s = sqrt(trace + tf2Scalar(1.0));
			...
			else 
			{
				...
				tf2Scalar s = sqrt(m_el[i][i] - m_el[j][j] - m_el[k][k] + tf2Scalar(1.0));
		...
		void getEulerYPR(tf2Scalar& yaw, tf2Scalar& pitch, tf2Scalar& roll, unsigned int solution_number = 1) const
		{
			...
			if (fabs(m_el[2].x()) >= 1)
			{
				...
				tf2Scalar delta = atan2(m_el[2].y(),m_el[2].z());
		...
		void diagonalize(Matrix3x3& rot, tf2Scalar threshold, int maxSteps)
		{
			...
			for (int step = maxSteps; step > 0; step--)
			{
				...
				int q = 1;
				int r = 2;
				tf2Scalar max = fabs(m_el[0][1]);
				tf2Scalar v = fabs(m_el[0][2]);
				if (v > max)
				{
					q = 2;
					r = 1;
				...
				tf2Scalar t = threshold * (fabs(m_el[0][0]) + fabs(m_el[1][1]) + fabs(m_el[2][2]));
				if (max <= t)
				{
					if (max <= DBL_EPSILON * t)
					{
						return;
					}
					step = 1;
				...
				tf2Scalar cos;
				tf2Scalar sin;
				if (theta2 * theta2 < tf2Scalar(10 / DBL_EPSILON))
				{
