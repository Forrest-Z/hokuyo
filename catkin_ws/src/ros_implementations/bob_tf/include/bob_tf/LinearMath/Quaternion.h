#ifndef _BOB_TF2_QUATERNION_H_
#define _BOB_TF2_QUATERNION_H_
...
public:
  ...
  Quaternion(const tf2Scalar& yaw, const tf2Scalar& pitch, const tf2Scalar& roll) __attribute__((deprecated))
	{ 
#ifndef _BOB_TF2_EULER_DEFAULT_ZYX
   ...
   * @param angle The magnitude of the rotation in Radians */
	void setRotation(const Vector3& axis, const tf2Scalar& angle)
	{
		...
		tf2Scalar s = sin(angle * tf2Scalar(0.5)) / d;
		setValue(axis.x() * s, axis.y() * s, axis.z() * s, 
			cos(angle * tf2Scalar(0.5)));
	}
   ...
   * @param roll Angle around Z */
	void setEuler(const tf2Scalar& yaw, const tf2Scalar& pitch, const tf2Scalar& roll)
	{
		...
		tf2Scalar cosYaw = cos(halfYaw);
		tf2Scalar sinYaw = sin(halfYaw);
		tf2Scalar cosPitch = cos(halfPitch);
		tf2Scalar sinPitch = sin(halfPitch);
		tf2Scalar cosRoll = cos(halfRoll);
		tf2Scalar sinRoll = sin(halfRoll);
   ...
   * @param q The other quaternion */
	...
	{
		tf2Scalar s = sqrt(length2() * q.length2());
  ...
  /**@brief Return the angle of rotation represented by this quaternion */
	...
	{
		tf2Scalar s_squared = tf2Scalar(1.) - pow(m_floats[3], tf2Scalar(2.));
		if (s_squared < tf2Scalar(10.) * DBL_EPSILON) //Check for divide by zero
			return Vector3(1.0, 0.0, 0.0);  //! Arbitrary
		tf2Scalar s = sqrt(s_squared);
   ...
   * Slerp interpolates assuming constant velocity.  */
	Quaternion slerp(const Quaternion& q, const tf2Scalar& t) const
	{
		...
		{
			tf2Scalar d = tf2Scalar(1.0) / sin(theta);
			tf2Scalar s0 = sin((tf2Scalar(1.0) - t) * theta);
			tf2Scalar s1 = sin(t * theta);   
                        if (dot(q) < 0) //! Take care of long angle case see http://en.wikipedia.org/wiki/Slerp
