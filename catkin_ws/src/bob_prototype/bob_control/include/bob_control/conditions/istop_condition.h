...
namespace bob
{
	...
	class IStopCondition
	{
		...
		public:
			...
			//! \brief A shared pointer to a IStopCondition object
			typedef boost::shared_ptr<IStopCondition> shared_ptr;

			//! \brief Basic constructor. Note that the condition always starts off not satisfied
			IStopCondition() : satisfied(false) {}

			//! \brief This function is called in the control loop. It updates condition and returns
			//! the updated value. If the condition has been triggered, it will be detected.
			inline bool isSatisfied(const ISensorHandle& sensorHandle)	
			{
				if (this->condition(sensorHandle))
					satisfied = true;

				return satisfied;
			}

			//! \brief Check if the condition was satisfied in the control loop. This will not	
			...
			//! \brief Resets the condition to false
			void reset()
			{
				satisfied = false;
			}

			//! \brief Virtual destructor allows derived classes to be deleted via pointer to base class
			virtual ~IStopCondition()
			{}

		private:

			//! \brief This method is overloaded in derived classes to define the condition
